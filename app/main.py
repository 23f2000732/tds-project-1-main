...
def process_request(data):
    ...
    print(
        f"⚙ Starting background process for task {task_id} (round {round_num})")
    ...
    if round_num == 2:
        try:
            ...
            prev_readme = readme.decoded_content.decode(
                "utf-8", errors="ignore")
    ...
    for att in saved_info:
        ...
        try:
            ...
            if att["mime"].startswith("text") or att["name"].endswith(
                (".md", ".csv", ".json", ".txt")
            ):
                text = content_bytes.decode("utf-8", errors="ignore")
                create_or_update_file(
                    repo, path, text, f"Add attachment {path}")
            else:
                create_or_update_binary_file(
                    repo, path, content_bytes, f"Add binary {path}"
                )
                b64 = base64.b64encode(content_bytes).decode("utf-8")
                create_or_update_file(
                    repo,
                    f"attachments/{att['name']}.b64",
                    b64,
                    f"Backup {att['name']}.b64",
                )
