...
def decode_attachments(attachments):
    ...
    for att in attachments or []:
        name = att.get("name") or uuid4().hex + ".dat"
        url = att.get("url", "")
        ...
        try:
            ...
            path = TMP_DIR.joinpath(name)
            with open(path, "wb") as f:
                f.write(data)
            saved.append(
                {"name": name, "path": str(
                    path), "mime": mime, "size": len(data)}
            )
...
def summarize_attachment_meta(saved):
    ...
    for s in saved:
        ...
        mime = s.get("mime", "")
        try:
            if mime.startswith("text") or nm.endswith(TEXT_TYPES):
...
def generate_readme_fallback(
    brief: str, checks=None, attachments_meta=None, round_num=1
):
    checks_text = "\\n".join(checks or [])
...
def generate_app_code(
    brief: str, attachments=None, checks=None, round_num=1, prev_readme=None
):
    """
    ...
    if round_num == 2 and prev_readme:
        context_note = f"\n### Previous README.md:\n{prev_readme}\n\nRevise and enhance this project according to the new brief given.\n"

    user_prompt = f"""
You are a professional senior web developer. Given the task brief
and attachments, create a simple static web application using
HTML, CSS, and JavaScript, that will be hosted on GitHub Pages.
...
"""
    ...
    if "---README.md---" in text:
        ...
        readme_part = generate_readme_fallback(
            brief, checks, attachments_meta, round_num
        )
